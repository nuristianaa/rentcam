  ...
  <q-dialog v-model="model">
    <q-card style="width: 900px; max-width: 85vw">
      <q-bar style="position: sticky; top: 0; width: 100%; z-index: 5; justify-content: space-between">
        <div>{{ props.title || 'Table View' }}</div>
        <q-btn v-close-popup flat icon="close" size="sm" />
      </q-bar>
      <q-card-section class="q-pa-md">
        <q-table flat virtual-scroll binary-state-sort :style="'width: 100%; height: 72vh; min-height: 300px;'" :loading="table.loading" :row-key="'id'" :rows="table.data" :columns="table.columns" :rows-per-page-options="[0]" :dense="true" v-model:pagination="table.pagination">
...
const API = new Api()
const columns = props.metaTable.table(Helper, Constant, Lang).map((el: any) => {
  const row = { ...el }
  // Edit here if needed
  return row
})
