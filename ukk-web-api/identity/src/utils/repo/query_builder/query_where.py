...
def query_where(b, model, q):
  ...
  for item in filter_list(b.where):
    ...
    q = q.where(get_column(model, col) == val)
  for item in filter_list(b.eq):
    col, val = parse_item(item)
    q = q.where(get_column(model, col) == to_number(val))
  return q
...
def query_like(b, model, q):
  ...
  for item in filter_list(b.like):
    ...
    if raw_val in BOOL_MAP:
      ...
      q = q.where(col_expr.isnot(None)) if raw_val == "notnull" else q.where(col_expr.is_(mapped))
    else:
      q = q.where(get_column(model, col).ilike(f"%{raw_val}%"))
...
def query_notlike(b, model, q):
  ...
  for item in filter_list(b.notlike):
    col, val = parse_item(item)
    q = q.where(get_column(model, col).notilike(f"%{val}%"))
  return q
...
def query_start(b, model, q):
  ...
  for item in filter_list(b.start):
    col, val = parse_item(item)
    q = q.where(get_column(model, col).ilike(f"{val}%"))
  return q
...
def query_in(b, model, q):
  ...
  for item in filter_list(b.in_):
    ...
    q = q.where(get_column(model, col).in_(vals))
  return q
...
def query_between(b, model, q):
  ...
  for item in filter_list(b.between):
    ...
    q = q.where(get_column(model, col).between(start, end))
  return q
...
def query_notbetween(b, model, q):
  ...
  for item in filter_list(b.notbetween):
    col, val = parse_item(item)
    start, end = val.split(" to ", 1)
    q = q.where(not_(get_column(model, col).between(start, end)))
  return q
...
def query_trash(b, model, q):
  if "deleted_at" in model:
    ...
    if b.trash: q = q.where(col.isnot(None))
    elif not b.all: q = q.where(col.is_(None))
  return q
