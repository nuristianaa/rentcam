...
class User(Base):
  ...
  # menu_favorites: Mapped[list[dict] | None] = mapped_column(JSONB, nullable=True)
  # table_configs: Mapped[list[dict] | None] = mapped_column(JSONB, nullable=True)
  # activated_at: Mapped[datetime | None] = mapped_column(DateTime, default=func.now())
  ...
  # roles = relationship("Role", secondary="auth.user_roles", back_populates="users")  # Removed: Role not available in rental context
